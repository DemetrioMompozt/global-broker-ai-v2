...
export function validatePortfolioRiskGuard(input: { cfdSymbol: string; riskPercent: number; assetClass: string }) {
  ...
  if (open.length >= tradingConfig.maxOpenPositions) reasons.push('Maximo de 5 posiciones abiertas alcanzado.')
  ...
  if (input.assetClass === 'CRYPTO_CFD' && cryptoOpen >= tradingConfig.maxCryptoPositions) reasons.push('Maximo de 1 posicion cripto CFD alcanzado.')
  return {
