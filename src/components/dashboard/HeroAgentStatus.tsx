  ...
  return (
    <section className="hero">
      <div>
        ...
        <h2>{title}</h2>
        <p>{subtitle}</p>
        <p className="muted">Ultima evaluacion: {status.agent.lastEvaluationAt ? new Date(status.agent.lastEvaluationAt).toLocaleTimeString() : 'pendiente'} · Proxima: {status.agent.nextEvaluationAt ? new Date(status.agent.nextEvaluationAt).toLocaleTimeString() : 'pendiente'}</p>
      </div>
      <div className="hero-actions">
        <button onClick={() => void startAgent().then(onRefresh)}>Iniciar agente paper</button>
        <button className="secondary" onClick={() => void stopAgent().then(onRefresh)}>Detener</button>
      </div>
    </section>
  )
}
